...
import TrendingSkeleton from "./LatestInSkeleton"
  ...
  const [products, setProducts] = useState<Product[]>([])
    ...
    useEffect(() => {
      const fetchProducts = async () => {
        try {
         const response =
           await client.fetch(`*[_type == "product" && isNew == true] | order(_createdAt desc)[0...8]{
            title, 
            price, 
            tags[], 
            _id, 
   ...
   if (loading) return <TrendingSkeleton itemCount={8} />
    if (error) return <div className="p-8 text-center text-red-500">{error}</div>
  
    return (
      <div className="scrollbar-hide grid xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-8">
        {products.map((item) => (
          <div
            key={item._id}
